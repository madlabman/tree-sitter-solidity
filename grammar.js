...
module.exports = grammar({
  ...
  rules: {
    ...
    //  -- [ Directives ] --
    _directive: ($) =>
      choice($.pragma_directive, $.import_directive, $.using_directive),
          ...
          seq(
            ...
            commaSep1(
              seq(
                alias($.user_defined_type, $.type_alias),
                optional(
                  seq("as", field("operator", $.user_definable_operator)),
                ),
              ),
            ),
            "}",
    ...
    _assembly_flags: ($) => seq("(", commaSep1($.string), ")"),
    ...
    unchecked: ($) => "unchecked",
    block_statement: ($) =>
      seq(optional($.unchecked), "{", repeat($._statement), "}"),
    variable_declaration_statement: ($) =>
      prec(
        1,
        seq(
